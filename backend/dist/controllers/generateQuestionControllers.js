...
const addTestToDB = async (exam, subject, topic, questions) => {
    try {
        const existingTests = await Test_model_1.TestModel.find({ examType: exam, subject, topic });
        const isDuplicate = existingTests.some((test) => {
            return (0, lodash_isequal_1.default)(test.questions.map(q => ({
                question: q.question,
                options: q.options,
        ...
        const newtest = await Test_model_1.TestModel.create(test);
        console.log(test);
...
const addtoquestioncollection = async (exam, subject, topic, questions) => {
    try {
        ...
        for (const formattedQuestion of formattedQuestions) {
            ...
            if (!existingQuestion) {
                await Question_model_1.Question.create(formattedQuestion);
